...
/**
 * This interface encapsulates all supported requests to the EVE servers.
 * Implementations of this interface may not implement all functions and are
 * allowed to throw an UnsupportedOperationException for those functions not
 * supported.
 */
public interface IEveXmlApi {
	 ...
	 * @return a handle for making API requests.
	 */
	public ICharacterAPI getCharacterAPIService(int keyID, String vCode,
			long characterID);
	 ...
	 * @return a handle for making API requests.
	 */
	public ICorporationAPI getCorporationAPIService(int keyID,
			String vCode, long characterID);

	/**
	 * Request a handle for making eve API requests. All but one of these
	 * requests do not require credentials. The one request which does require
	 * credentials (CharacterInfo) accepts these credentials on the methods
	 * call.
	 * 
	 * @return a handle for making API requests.
	 */
	...
	// TODO: define APIs for requesting images from the image server
