...
public class CharUpcomingCalendarEventsConverter extends ResponseConverter {
  ...
  public static String convert(
    ...
    if (response.size() == 0)
      ...
      builder.append(">\n");
      builder
          .append("<rowset name=\"upcomingEvents\" key=\"eventID\" columns=\"eventID,ownerID,ownerName,eventDate,eventTitle,duration,importance,response,eventText,ownerTypeID\">\n");
      ...
      for (IUpcomingCalendarEvent next : response) {
        ...
        builder.append("\" eventDate=\"").append(toEveDateFormat(next.getEventDate()));
        builder.append("\" eventTitle=\"").append(next.getEventTitle());
        builder.append("\" duration=\"").append(next.getDuration());
        builder.append("\" importance=\"").append(next.isImportant() ? 1 : 0);
        builder.append("\" response=\"").append(next.getResponse());
        String eventText = next.getEventText().replace("<", "&lt;").replace(">", "&gt;").replace("\"", "&quot;");
        builder.append("\" eventText=\"").append(eventText);
        builder.append("\" ownerTypeID=\"").append(next.getOwnerTypeID());
        builder.append("\" />\n");
      }
