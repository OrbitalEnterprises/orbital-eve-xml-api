...
public class CorpWalletJournalConverter extends ResponseConverter {
	...
	public static String convert(ICorporationAPI request,
		...
		builder.append("<rowset name=\"entries\" key=\"refID\" columns=\"date,refID,refTypeID,ownerName1,ownerID1,ownerName2,ownerID2,argName1,argID1,amount,balance,reason\"");
		...
		if (response.size() == 0)
			...
			for (IWalletJournalEntry next : response) {
				...
				builder.append("\" amount=\"").append(
						String.format("%.2f", next.getAmount()));
